...
namespace torchmedia::audio {
    enum SignalWiseCompareMode { MSE, MAE, SNR };
    ...
    auto inline signal_wise_almost_equal(const torch::Tensor &left, const torch::Tensor &right, float threshold = 1e-7,
        ...
        if (mode == SignalWiseCompareMode::MSE) {
            compared = torch::nn::functional::mse_loss(
                    left, right, torch::nn::functional::MSELossFuncOptions().reduction(torch::kMean));
            ...
            compared = torch::nn::functional::l1_loss(
                    left, right, torch::nn::functional::L1LossFuncOptions().reduction(torch::kMean));
        }
